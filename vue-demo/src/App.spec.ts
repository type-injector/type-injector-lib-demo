...
describe('App', () => {
  ...
  describe('with globally provided type injector', () => {
    beforeEach(() => {
      wrapper = mount(App, {
        global: {
          provide: {
    ...
    it('should nested usage', () => {
      const directUsage = wrapper.find('.container > section:nth-child(2)');
      expect(directUsage.find('h2').text()).to.match(/in child/i);
      expect(directUsage.find('p').text()).to.match(/Hello test!/);
    });
  ...
  describe('without globally provided type injector', () => {
    beforeEach(() => {
      wrapper = mount(App);
    });

    it('should use global injector as fallback', () => {
      const directUsage = wrapper.find('.container > section:nth-child(1)');
      expect(directUsage.find('h2').text()).to.match(/direct usage/i);
      expect(directUsage.find('p').text()).to.match(/Hello vue.js!/);
    });
  });
});
